const PT_DATA = {
  menu: [
    'HOME',
    'SERVIÇOS',
    'SOBRE NÓS',
    'CONTATO',
    'LOGIN'
  ],
  sections: {
    services: [
      'SERVIÇOS',
      'Cursos Online',
      'Cursos Presenciais',
      'Veja mais'
    ],
    about: [
      'SOBRE NÓS',
      'A Head Start Courses é uma empresa com sede em Dublin formada por Bartira Augelli, profissional renomada e focada na área de treinamentos na Irlanda, incluindo cursos de Food Safety (HACCP - Level 1) e Barista, além de oferecer suporte para o desenvolvimento de currículo dos estudantes. Ambos são oferecidos no ambiente presencial ou online, em inglês ou português, contribuindo especialmente àqueles que precisam de um conteúdo eficaz e claro para conseguirem um emprego na Irlanda.',
      'Com uma linguagem simples, dinâmica e envolvente, a Head Start Courses auxilia através do treinamento adequado, com o objetivo de desenvolver a confiança necessária para que o estudante se destaque no mercado de trabalho, contribuindo, assim, com sua jornada pessoal e profissional.',
      'MISSÃO',
      'Entregar conhecimento de forma leve e eficaz, desenvolvendo pessoal e profissionalmente o aluno.',
      'VISÃO',
      'Ser referência na formação de profissionais destaques no setor alimentício da Irlanda, contribuindo com a vida dos imigrantes e com o país.',
      'VALORES',
      'Honestidade, Integridade, Respeito e Inclusão',
      'QUEM É BARTIRA?',
      '“Meu negócio surgiu quando percebi que meu conhecimento e experiência poderiam gerar um  impacto positivo na vida das pessoas, principalmente intercambistas como eu. Desde então, minha alegria é trabalhar com o que amo e saber que contribuo para o start de muitos profissionais.”',
      '(BARTIRA)',
      'Com quase 10 anos de experiência na área alimentícia da Irlanda, Bartira Augelli é Empreendedora, Professora e Mestre pela Trinity College Dublin (TCD). É fundadora da Head Start Courses e do renomado “Barista Course By Bartira”, que, em 7 anos, já qualificou mais de 5 mil profissionais, além de ser proprietária da estilosa cafeteria localizada no centro de Dublin.',
      'A profissional brasileira é Bacharel pela International College Dublin (ICD); e “BA (Honours) Business Studies” em Negócios e Treinamento, com diploma pela School of Education - Trinity College Dublin em “Mentoring Training”, além de ter certificação em “Advanced HACCP Design and Validation based on Risk Assessment - Level 6” pelo QQI.',
      'Bartira is the founder of Head Start Courses and of the renowned “Barista Course By Bartira”, that in 7 years already has qualified more than 5000 professionals. She also is the owner of the stylish coffee shop located in British Studies Centres, Dublin.',
      'Bartira também faz parte e atua como mentora no “Career LEAP” Work Readiness Training Programme, programa criado pela Trinity College Dublin e negócios locais com o propósito de minimizar o desemprego entre jovens. Parte do seu tempo é dedicado para dar palestras, treinamentos e workshops, bem como para trabalhos sociais, através de treinamentos para adolescentes que fazem parte de um grupo socioeconômico tradicionalmente desafiador em áreas carentes de Dublin.',
      '',
      'Transformando vidas atráves da educação'
    ],
    contact: [
      'CONTATO',
      'Preencha o formulário abixo. Retornaremos assim que possível.',
      'Nome',
      'Email',
      'Mensagem',
      'Enviar',
      'Localização',
      'Você pode encontrar cursos da Head Start Courses em:',
      'Dúvidas? Estamos aqui para ajudar você. Fale com a gente!'
    ],
    login: [
      'Email',
      'Senha',
      'ou Crie uma nova conta',
      'Esqueceu a Senha?'
    ],
    security: [
      'Politica de Segurança',
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nemo, explicabo. Quos asperiores ullam ea illum beatae eligendi dicta, quis corrupti quam nemo ipsum itaque expedita ex magni, facilis similique quas?'
    ],
    cart: [
      'Carrinho',
      'Cursos no Carrinho',
      'CONFIRMAR',
      'Inserir Cupom',
      'APLICAR'
    ]
  }
};

export default PT_DATA;